...
class CustomshouseSpider(scrapy.Spider):
    ...
    def get_flash(self, response):
        ...
        for page in page_list:
            ...
            yield scrapy.Request(url=flash_url, callback=self.parse_data)
            yield scrapy.Request(url=flash_url, callback=self.get_flash_keyword)
    ...
    def get_flash_keyword(self):
        #获取海关快报的关键词(固定)
        item = ChinacustomsspiderItem()
        item['keyword'] = '中国海关; 统计; 快讯'
        yield item

    def get_monthly_magazine_keyword(self, response):
        #获取海关月报的关键词
        item = ChinacustomsspiderItem()
        dataupdate = response.xpath('//div[@class="detailTime]/text()').extract()
        dataupdate_str = self.remove_unused(dataupdate).strip('发布时间：')
    ...
    def get_monthly_magazine(self, response):
        ...
        for monthly_magazine_url in monthly_magazine_urls:
            ...
            yield scrapy.Request(url=monthly_magazine_url, callback=self.parse_data)
            yield scrapy.Request(url=monthly_magazine_url, callback=self.get_monthly_magazine_keyword)

    def get_custom_district(self, response):
        #获取关区统计的链接
        pages_text = response.xpath('//div[@id="ess_ctr126763_ListC_Info_AspNetPager"]/table/tr/td[1]//text()').extract()
